...
public partial class BookDoc_Admin_doctor_review : System.Web.UI.Page
{
    ...
    protected void GetUserReview()
    {
      if(con.State.ToString()=="Closed")
        ...
        SqlDataAdapter sda = new SqlDataAdapter("Select us.*,doc.*,re.*,re.id as reid from tbl_signup us inner join tbl_user_feed re on re.u_email=us.u_hakkimid inner join tbl_doctor doc on doc.d_hakkimid=re.d_email where re.d_email='" + doctorid + "'", con);
        sda.Fill(dt);
        if (dt.Rows.Count > 0)
        {
